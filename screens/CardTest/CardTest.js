...
class CardTest extends React.Component {
  ...
  addEvent = (id, name, color, side) => {
    ...
    this.setState({
      responseModalVisible: true,
      idToDelete: id,
      totalEvents: this.state.totalEvents + 1,
      roundEvents: this.state.roundEvents + 1,
    });
    if (this.state.roundEvents == 9) {
      console.log("cambio criterio");
      if (this.state.evaluation == "color") {
        this.setState({
          evaluation: "shape",
          roundEvents: 0,
  ...
  addCatch = () => {
    ...
    results.push({
      ...
      criterio: criterio,
      catchPersistence: catchPersistence,
      mistakePersistence: 0,
      round: this.state.totalEvents + 1,
      event: "catch",
      timeInMs: time,
    });
    this.setState({
      catches: this.state.catches + 1,
      lastEvent: "catch",
  ...
  addMistake = () => {
    ...
    var results = this.state.roundResults;
    var criterio = this.state.evaluation;
    if (this.state.roundEvents != 0) {
      mistakePersistence = mistakePersistence + 1;
    }
    results.push({
      user: this.state.patientNumber,
      interviewer: this.state.interviewerNumber,
      criterio: criterio,
      catchPersistence: 0,
      mistakePersistence: mistakePersistence,
      round: this.state.totalEvents + 1,
      event: "mistake",
      timeInMs: time,
    });
  ...
  setResponseModalInvisible = () => {
    var result = this.state.cards.filter(
      (item) => item.id != this.state.idToDelete
    );
  ...
  render() {
    const listado = this.state.cards.map((item) => (
      <CardContainer
        key={item.id}
        id={item.id}
        name={item.name}
    ...
    return (
      <>
        <Modal
          animationType="slide"
          visible={this.state.finishedGameModalVisible}
        >
          <ReturnHomeComponent navigation={this.props.navigation} />
        </Modal>
        {!this.state.finishedTutorial ? (
          <CardTutorial
            ...
            {!this.state.showCross ? (
              <>
                ...
                >
                  ...
                  >
                    <TouchableOpacity
                      style={mainPage.button}
                      onPress={this.setInvisible}
                    >
                      <Text style={instructions.buttonText}>
                        {" "}
                        Comenzar test
                      </Text>
                    </TouchableOpacity>
                    ...
                    {this.state.lastEvent == "catch" ? (
                      <View style={instructions.popupModalCorrectContainer}>
                        ...
                        <View style={instructions.buttonModalContainer}>
                          <TouchableOpacity
                            style={[
                              instructions.emptyButton,
                              { alignSelf: "center" },
                            ]}
                            onPress={this.setResponseModalInvisible}
                          >
                            <Text style={instructions.emptyButtonText}>
                              {" "}
                              Siguiente carta
                            </Text>
                          </TouchableOpacity>
                      ...
                      <View style={instructions.popupModalIncorrectContainer}>
                        ...
                        <View
                          style={[
                            instructions.buttonModalContainer,
                            { flexDirection: "row" },
                          ]}
                        >
                          <TouchableOpacity
                            style={[
                              instructions.emptyDangerButton,
                              { alignSelf: "center" },
                            ]}
                            onPress={this.setResponseModalInvisible}
                          >
                            <Text style={instructions.emptyDangerButtonText}>
                              Siguiente carta
                            </Text>
                          </TouchableOpacity>
                        </View>
